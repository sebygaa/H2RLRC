...
def isomix(P_in,T):
    ...
    for ii in range(len(P_in)):
        ...
        #p_tmp[ind_tmp] = 0
        pp.append(p_tmp)
    q1 = 1*pp[0]*0.1/(1 + 0.1*pp[0] + 0.5*pp[1])
    q2 = 5*pp[1]*0.5/(1 + 0.1*pp[0] + 0.5*pp[1] )
    return [q1, q2]
...
plt.grid(linestyle = ':')
plt.savefig('Savedfig_y_frac_binary.png', dpi = 100, 
            bbox_inches = 'tight')
...
plt.savefig('Savedfig_q_res_binary.png', dpi = 100,
            bbox_inches = 'tight')
...
for ii in range(len(t_dom))[::2000]:
    n_ls = n_ls+1
...
plt.ylabel('gas phase mole fraction (mol/mol)', fontsize = 12)
plt.grid(linestyle = ':')
plt.savefig('Savedfig_y_frac_end_binary.png', dpi = 100,
            bbox_inches = 'tight')
...
plt.ylabel('solid phase mole fraction (mol/kg)', fontsize = 12)
plt.grid(linestyle = ':')
plt.savefig('Savedfig_q_res_end_binary.png', dpi = 100,
            bbox_inches = 'tight')
